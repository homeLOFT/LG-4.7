 ...
 * @version    2b39e63712da5477e1aaf5cfa80d1370f583bce9, v63 (xcart_4_7_0), 2015-02-17 23:56:28, mod_ARB.php, Yuriy
...
function ab_int_ratings($allowed_shipping_methods, $userinfo, $debug, $params, $orig_address, $packages)
{
    ...
    $has_states = (func_query_first_cell("SELECT COUNT(*) FROM $sql_tbl[states] WHERE country_code = '$userinfo[s_country]'") > 0);

    $rc_address = (!empty($userinfo['s_address'])) ? htmlspecialchars($userinfo['s_address']) : 'N/A';
    $rc_address_2 = htmlspecialchars($userinfo['s_address_2']);
    $rc_city = (!empty($userinfo['s_city'])) ? $userinfo['s_city'] : 'N/A';
    $rc_state = ($has_states ? $userinfo['s_state'] : (strlen($userinfo['s_state']) > 10 ? 'Other' : $userinfo['s_state']));
