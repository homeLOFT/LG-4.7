...
function func_calculate($cart, $products, $user, $login_type, $paymentid = NULL)
{
    ...
    if (!is_null($paymentid)) {
        ...
        $_payment_surcharge = func_payment_method_surcharge($return['total_cost'], $paymentid, $cart);
...
function func_calculate_taxes(&$products, $customer_info, $shipping_cost)
{
    ...
    foreach ($products as $k => $product) { // {{{
        ...
        $products[$k]['taxes'] = $product_taxes_array[$k] = func_get_product_taxes($products[$k], @$customer_info['id'], true, '', array(), $customer_info);
    ...
    return func_calculate_taxes_by_formula($shipping_cost, $product_taxes_array);
}

/**
 * This function calculates taxes according to formula
 */
function func_calculate_taxes_by_formula($shipping_cost, $product_taxes_array) { // {{{
    $taxes = array(
        'total'       => 0,
        'shipping'    => 0,
    );
...
{
    global $sql_tbl, $cart, $logged_userid, $sql_obj;
    ...
    if (!empty($user)) {
        ...
        $res = $sql_obj->query_delayed(array(
            'query' => "REPLACE INTO $sql_tbl[customer_saved_carts] SET saved_cart='".addslashes($serialized_cart)."', userid='$user'",
