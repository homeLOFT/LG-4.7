{*
c0b7a742f56d4cd9d5875fb9a121be48c4674208, v5 (xcart_4_7_0), 2014-12-20 17:15:41, debug.tpl, aim
...
{if isset($_smarty_debug_output) and $_smarty_debug_output eq "html"}
  <table width="100%">
    ...
    {section name=templates loop=$_debug_tpls}
      <tr bgcolor="{if $smarty.section.templates.index is even}#eeeeee{else}#fafafa{/if}">
        <td colspan="2">
          <tt>
            ...
            {if isset($_debug_tpls[templates].exec_time)}
              <font size="-1"><i>({$_debug_tpls[templates].exec_time|string_format:"%.5f"}){if $smarty.section.templates.index eq 0} (total){/if}</i></font>
            {/if}
          </tt>
        </td>
      </tr>
    ...
    {section name=vars loop=$_debug_keys}
      <tr bgcolor="{if $smarty.section.vars.index is even}#eeeeee{else}#fafafa{/if}">
        <td valign="top">
    ...
    {section name=config_vars loop=$_debug_config_keys}
      <tr bgcolor="{if $smarty.section.config_vars.index is even}#eeeeee{else}#fafafa{/if}">
...
try {ldelim}
  ...
  if(_smarty_console) {ldelim}
    ...
    {section name=templates loop=$_debug_tpls}
      _smarty_console.document.write('<tr bgcolor="{if $smarty.section.templates.index is even}#eeeeee{else}#fafafa{/if}"><td colspan="2"><tt>{section name=indent loop=$_debug_tpls[templates].depth}&nbsp;&nbsp;&nbsp;{/section}<font color="{if $_debug_tpls[templates].type eq "template"}brown{elseif $_debug_tpls[templates].type eq "insert"}black{else}green{/if}">{$_debug_tpls[templates].filename|escape:html|wm_remove|escape:javascript}</font>{if isset($_debug_tpls[templates].exec_time)} <font size="-1"><i>({$_debug_tpls[templates].exec_time|string_format:"%.5f"}){if $smarty.section.templates.index eq 0} (total){/if}</i></font>{/if}</tt></td></tr>');
    {sectionelse}
    ...
    {section name=vars loop=$_debug_keys}
      _smarty_console.document.write('<tr bgcolor="{if $smarty.section.vars.index is even}#eeeeee{else}#fafafa{/if}"><td valign="top"><tt><font color="blue">{ldelim}${$_debug_keys[vars]}{rdelim}</font></tt></td><td nowrap="nowrap"><tt><font color="green">{$_debug_vals[vars]|@debug_print_var|wm_remove|escape:javascript}</font></tt></td></tr>');
    {sectionelse}
    ...
    {section name=config_vars loop=$_debug_config_keys}
      _smarty_console.document.write('<tr bgcolor="{if $smarty.section.config_vars.index is even}#eeeeee{else}#fafafa{/if}"><td valign="top"><tt><font color="maroon">{ldelim}#{$_debug_config_keys[config_vars]}#{rdelim}</font></tt></td><td><tt><font color="green">{$_debug_config_vals[config_vars]|@debug_print_var|wm_remove|escape:javascript}</font></tt></td></tr>');
    {sectionelse}
