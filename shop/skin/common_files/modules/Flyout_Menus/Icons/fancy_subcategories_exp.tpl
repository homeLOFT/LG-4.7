{*
c663d7ce6064c44dbbdc3324a502b67d54eab630, v4 (xcart_4_7_0), 2014-12-23 18:57:26, fancy_subcategories_exp.tpl, aim
...
<ul class="fancycat-icons-level-{$level}">
  ...
  {foreach from=$categories_menu_list item=c key=catid name=$loop_name}
    ...
    <li id="cat-layer-{$c.categoryid}"{interline name=$loop_name foreach_iteration=$c@iteration foreach_total=$c@total additional_class=$additional_class}>
      {strip}
      {if $config.Flyout_Menus.icons_disable_subcat_triangle eq 'Y' and $c.subcategory_count gt 0}
