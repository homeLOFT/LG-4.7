{*
16fd72c950254a8d4861815dac0d6147af93e43f, v12 (xcart_4_7_0), 2014-12-23 10:13:26, users.tpl, aim
...
{foreach from=$users item=u name=users}
  ...
  <tr{interline name=users foreach_iteration="`$smarty.foreach.users.iteration`" foreach_total="`$smarty.foreach.users.total`" class=TableSubHead}>
    ...
    <td>
      <a href="{if $u.id eq $logged_userid}register.php?mode=update{else}user_modify.php?user={$u.id|escape:"url"}&amp;usertype={$u.usertype}{$pagestr|amp}{/if}" title="{$lng.lbl_modify_profile|escape}"><font class="ItemsList">{$uname|default:$u.login}</font></a>
      {if $uname ne $u.email}
        <br />{$u.email}
      {/if}
    </td>
