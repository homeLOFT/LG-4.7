{*
c0b7a742f56d4cd9d5875fb9a121be48c4674208, v2 (xcart_4_7_0), 2014-12-20 17:15:41, snapshots.tpl, aim
...
{if $snapshots[cwi].no_file ne "Y"}
  <option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot eq "" and $smarty.section.cwi.last) or $search_prefilled.snapshot eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot|wm_remove|escape}</option>
{/if}
{/section}
  </select>
  </td>
...
{section name=cwi loop=$snapshots}
{if $snapshots[cwi].no_file ne "Y"}
  <option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot1 eq "" and $smarty.section.cwi.last) or $search_prefilled.snapshot1 eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot|wm_remove|escape}</option>
{/if}
{/section}
  </select>
  </td>
...
{section name=cwi loop=$snapshots}
{if $snapshots[cwi].no_file ne "Y"}
  <option value="{$snapshots[cwi].time}"{if ($search_prefilled.snapshot2 eq "" and $smarty.section.cwi.last) or $search_prefilled.snapshot2 eq $snapshots[cwi].time} selected="selected"{/if}>{$snapshots[cwi].time|date_format:$config.Appearance.datetime_format} - {$snapshots[cwi].descr|default:$lng.lbl_noname_snapshot|wm_remove|escape}</option>
{/if}
{/section}
  </select>
  </td>
