  ...
  {else}
    ...
    {foreach from=$sort_fields key=name item=field}
      ...
      <span class="search-sort-cell">
        {if $name eq $selected}
          <a href="{$url}&amp;sort={$name|amp}&amp;sort_direction={if $direction eq 1}0{else}1{/if}" title="{$lng.lbl_sort_by|escape}: {$field|escape}" class="search-sort-link {if $direction}fa fa-arrow-circle-o-down{else}fa fa-arrow-circle-o-up{/if}">{$field|escape}</a>
        {else}
